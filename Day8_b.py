def day8_b(input):
    ...
    for s in instructions:
        # Put all the changed registers into a register dictionary
        registers[s.split()[0]] = 0
    
    for s in instructions:
        temp = s.split("if")
        action = temp[0]
        condition = temp[1]
        if handle_condition(condition, registers):
            tempVal = handle_action(action, registers)
            if tempVal > maxVal:
                maxVal = tempVal
    
    print(maxVal)

def handle_condition(input, registers):
    temp = input.split()
    register = temp[0]
    condition = temp[1]
    value = int(temp[2])
...
def handle_action(input, registers):
    temp = input.split()
    register = temp[0]
    operator = temp[1]
    value = int(temp[2])
