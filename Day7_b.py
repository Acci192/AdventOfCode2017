...
def day7_b(input):
    ...
    for s in split_input:
        node = s.split()[0]
        units[node] = s.split()[1:]
    
    # transforming the (weight) into a integer
    for o in units:
        units[o][0] = int(units[o][0][1:-1])
        if "->" in units[o]:
            units[o].remove("->")
        for i in range(1, len(units[o])):
            units[o][i] = units[o][i].replace(",", "")
    
    new_imput = copy.deepcopy(units)
    ...
    while(cont):
        ...
        for o in units:
            valid = True
            local_weight = units[o][0]
            children_weights = []
            for i in range(1, len(units[o])):
                temp_string = units[o][i]
                if temp_string in leafs:
                    children_weights.append(leafs[temp_string])
                else:
                    valid = False
                    break
            if not valid:
                continue
            elif len(set(children_weights)) == 1:
                weight = children_weights[0]*len(children_weights) + local_weight
                leafs[o] = weight
                to_remove.append(o)
            else:
                unvalid_tree = o
                unvalid_weights = children_weights
                cont = False
                
    
    for i in unvalid_weights:
        if unvalid_weights.count(i) == 1:
            unique_index = unvalid_weights.index(i)
            unique_weight = i
        else:
            duplicate_weight = i
    
    difference = unique_weight - duplicate_weight
    
    
    failed_disc = new_imput[unvalid_tree][unique_index+1]
    
    temp_weight = new_imput[failed_disc][0]
    
    
    temp_weight -= difference
    
    
    print(temp_weight)    

day7_b("""bqyqwn (68) -> wscqe, cwxspl, syogw, xnxudsh
ddswb (34)
hnkvw (320)
